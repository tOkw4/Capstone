...
function saveScannedURL(url, result) {
    const timestamp = new Date().toLocaleString();
    chrome.storage.local.get('urlHistory', (data) => {
        const urlHistory = data.urlHistory || [];
        urlHistory.push({url, result, timestamp});

        chrome.storage.local.set({ urlHistory }, () => {
            console.log('URL scan saved successfully:', {url, result, timestamp});
        });
    });
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.status === "complete" && tab.active) {
        ...
        .then(data => {
            ...
            // Save the scanned URL and its details
            saveScannedURL(currentUrl, result, {
